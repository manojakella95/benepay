...
class FormValidatorService {
    ...
    static getDynamicRules(field) {
        ...
        rules.push({
            claimedCountryCd:field.claimedCountryCd
        })
        ...
        if (field.valid && validateStructureFromDB) {
            ...
            switch (event.target.name) {
                case 'IBAN':
                    ...
                    // IBANcheck = await this.isIBANCoherencyValid(value, rules[1].claimedCountryCd);
                ...
                case 'SWIFTBIC':
                    ...
                    // BICcheck = await this.isBICCoherencyValid(value, rules[1].claimedCountryCd);
                ...
                case 'AccountNo':
                    ...
                    field.valid = false;
                    field.errors.push('Account Number accepts only Numbers');
                    break;
    ...
    static async isIBANValid(input) {
        const result = await HTTP.get(`${urls.validateIBANStructure}/${input}`);
        if (result.data) {
            return result.data;
        }
    }

    static async isIBANCoherencyValid(input, countryCd) {
        console.log("country",countryCd)
        const result = await HTTP.get(`${urls.validateCoherency}?checkFor=${'IBAN'}&codeAsPerBICOrIBAN=${input}&countryCd=${countryCd}&category=International`);
        if (result.data) {
            return result.data;
        } 
    }

    static async isBICCoherencyValid(input, countryCd) {
        console.log("country",countryCd)

        const result = await HTTP.get(`${urls.validateCoherency}?checkFor=${'SWIFT BIC'}&codeAsPerBICOrIBAN=${input}&countryCd=${countryCd}&category=International`);
        if (result.data) {
            return result.data;
        }
    }
    ...
    static async isSWIFTBICValid(input) {
        const result = await HTTP.get(`${urls.validateBIC}/${input}`);
        if (result.data) {
            return result.data;
        }
    }
